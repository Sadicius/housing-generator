...
class TrasteroMinimumAreaValidator(ConstraintValidatorPort):
    ...
    def validate(self, layout: Layout) -> ValidationResult:
        ...
        for room in layout.rooms:
            ...
            if room.is_placed:
                cumple = meets_minimum_width(room.boundary.polygon, TRASTERO_ANCHO_LIBRE_MIN_M)
                if cumple is False:
                    violations.append(
                        f"'{room.id}': ancho libre por debajo del minimo de B.2.5 "
                        f"({TRASTERO_ANCHO_LIBRE_MIN_M:.2f}m)"
                    )
                elif cumple is None:
                    warnings.append(
                        f"'{room.id}': forma no rectangular, no se puede verificar el ancho "
                        f"libre de trastero (B.2.5)"
                    )
