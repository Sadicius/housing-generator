...
def test_soft_constraint_should_be_near_is_actually_satisfied_by_the_search():
    ...
    assert (
        layout.metadata["soft_penalty"] == 0.0
    )  # la preferencia blanda SI se satisface
...
def test_hard_constraint_never_loses_to_soft_even_when_tempting():
    ...
    assert (
        living.boundary.polygon.distance(garage.boundary.polygon) > 0
    )  # de verdad separadas
...
def test_vivienda_adosada_respects_medianera_sides_end_to_end():
    ...
    assert min_x == pytest.approx(
        0.0, abs=0.01
    )  # llega al linde oeste (medianera, sin retranqueo)
    assert max_x == pytest.approx(
        8.0, abs=0.01
    )  # llega al linde este (medianera, sin retranqueo)
...
def test_vivienda_accesible_end_to_end_all_scoped_rooms_admit_the_turning_circle():
    ...
    from housing_generator.interface.cli.main import (
