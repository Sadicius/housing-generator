...
def test_room_in_corner_has_two_exterior_sides():
    ...
    room = box(
        0, 0, 4, 4
    )  # esquina inferior izquierda: toca el lado izquierdo y el inferior del solar
...
def test_room_spanning_full_width_has_one_exterior_side_on_each_touching_edge():
    ...
    room = box(
        0, 0, 10, 3
    )  # toca el borde inferior en todo su ancho, y los laterales izq/der tambien
...
def test_short_contact_below_threshold_does_not_count():
    ...
    room = box(
        0, 9.9, 0.2, 10
    )  # esquina, pero con lados de solo 0.2m y 0.1m de contacto
...
def test_excluded_segment_does_not_count_as_exterior_contact():
    ...
    room = box(
        7, 0, 10, 3
    )  # toca el borde inferior (exterior real) Y el derecho (medianera)
...
def test_excluded_segment_on_a_side_the_room_does_not_touch_has_no_effect():
    ...
    assert (
        count_exterior_sides(room, lot, excluded_segments=[medianera_este]) == 2
    )  # abajo + izquierda
