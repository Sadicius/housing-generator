...
def test_overlapping_wet_room_passes():
    ...
    bath_above = _placed(
        "bath", RoomType.BATHROOM, box(1, 1, 3, 3)
    )  # solapa completamente

    layout = Layout(lot=_dummy_lot(), rooms=[bath_above], zones=[])
    result = NucleoHumedoVerticalValidator(
        reference_wet_boundaries=[kitchen_below]
    ).validate(layout)
...
def test_any_wet_type_matches_not_specific_type():
    ...
    bath_above = _placed(
        "bath", RoomType.BATHROOM, box(2, 2, 5, 5)
    )  # solape parcial real
...
def test_non_overlapping_wet_room_fails():
    kitchen_below = box(0, 0, 4, 4)
    bath_above = _placed(
        "bath", RoomType.BATHROOM, box(10, 10, 13, 13)
    )  # lejos, sin solape

    layout = Layout(lot=_dummy_lot(), rooms=[bath_above], zones=[])
    violations = (
        NucleoHumedoVerticalValidator(reference_wet_boundaries=[kitchen_below])
        .validate(layout)
        .violations
    )
...
def test_dry_rooms_are_never_checked():
    kitchen_below = box(0, 0, 4, 4)
    living_above = _placed(
        "living", RoomType.LIVING_ROOM, box(10, 10, 14, 14)
    )  # sin solape, pero no es humeda
...
def test_multiple_reference_wet_rooms_are_unioned():
    ...
    bath_above = _placed(
        "bath", RoomType.BATHROOM, box(5.5, 5.5, 6.5, 6.5)
    )  # solapa con la 2a, no la 1a
