...
TABLA_1_MAS_DE_CINCO: List[float] = [
    25,
    12,
    8,
    8,
    8,
]  # E1..E5 fijos si hay >5 estancias
...
class EstanciaMinimumAreaValidator(ConstraintValidatorPort):
    ...
    def validate(self, layout: Layout) -> ValidationResult:
        estancias = [
            r for r in layout.rooms if r.space_category == SpaceCategory.ESTANCIA
        ]
        if not estancias:
        ...
        num_estancias = (
            self._total_override if self._total_override is not None else len(ordenadas)
        )

        violations = self._check_rank_minimums(ordenadas, num_estancias)
        cuadrado_violations, warnings = self._check_estancia_mayor_cuadrado(
            estancias, ordenadas
        )
    ...
    def _check_rank_minimums(
        self, ordenadas: List[Room], num_estancias: int
    ) -> List[str]:
        ...
        for local_i, room in enumerate(ordenadas, start=1):
            puesto = (
                self._global_rank.get(room.id, local_i)
                if self._global_rank
                else local_i
            )
    ...
    def _check_estancia_mayor_cuadrado(
        ...
        mayor = next(
            (r for r in estancias if r.room_type == RoomType.LIVING_ROOM), None
        )
        if mayor is None:
        ...
        if mayor.is_placed:
            cabe = can_inscribe_square(
                mayor.boundary.polygon, CUADRADO_INSCRIBIBLE_ESTANCIA_MAYOR_LADO_M
            )
            if cabe is False:
                violations.append(
