...
def _placed_with_declared_area(room_id, room_type, polygon, area_declarada) -> Room:
    r = Room(
        ...
        name=room_id,
        room_type=room_type,
        dimensions=Dimensions(area_m2=area_declarada),
    )
...
def test_reproduces_the_real_case_found_by_the_user():
    ...
    pasillo = _placed_with_declared_area(
        "pasillo", RoomType.CORRIDOR, box(0, 0, 3.5, 2.0), area_declarada=4.0
    )
...
def test_within_tolerance_passes():
    ...
    room = _placed_with_declared_area(
        "r", RoomType.BEDROOM, box(0, 0, 4, 2.2), area_declarada=8.0
    )  # 8.8m2 generado
...
def test_exactly_at_threshold_passes():
    ...
    room = _placed_with_declared_area(
        "r",
        RoomType.BEDROOM,
        box(0, 0, 4, area_generada / 4),
        area_declarada=area_declarada,
    )
    layout = Layout(lot=_dummy_lot(), rooms=[room], zones=[])
    result = AreaObjetivoValidator().validate(layout)
    assert result.violations == []
...
def test_deviation_below_target_also_fails_not_just_above():
    ...
    room = _placed_with_declared_area(
        "r", RoomType.BEDROOM, box(0, 0, 2, 2), area_declarada=12.0
    )  # 4m2 generado
    layout = Layout(lot=_dummy_lot(), rooms=[room], zones=[])
    result = AreaObjetivoValidator().validate(layout)
    assert len(result.violations) == 1
...
def test_applies_to_circulation_types_too():
    ...
    room = _placed_with_declared_area(
        "r", RoomType.CORRIDOR, box(0, 0, 4, 4), area_declarada=4.0
    )  # 16m2 generado
    layout = Layout(lot=_dummy_lot(), rooms=[room], zones=[])
    result = AreaObjetivoValidator().validate(layout)
    assert len(result.violations) == 1
