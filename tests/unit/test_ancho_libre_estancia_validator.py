...
def test_living_room_below_2_70_fails():
    ...
    )  # 2m de ancho, por debajo de 2.70m
...
def test_dormitorio_individual_uses_2_00_threshold_when_area_below_12():
    ...
    bed.boundary = Boundary(
        polygon=box(0, 0, 2.00, 4.0)
    )  # justo 2.00m, cumple individual
...
def test_no_living_room_means_no_estancia_mayor_check_and_no_warning():
    ...
    bed.boundary = Boundary(
        polygon=box(0, 0, 0.5, 40.0)
    )  # absurdamente estrecho, pero no es dormitorio<12? es >=12
