...
## [ARCH:reorganizacion-docs] Fase 2: docs/ reorganizada por temas

Continuación de la fase 1 (mover `docs/visualizador` a `html/`). La
"Referencia técnica por componente" de `architecture.md` (46
etiquetas `[ARCH:tag]` únicas, 49 secciones, 3280 líneas) se
distribuyó en 18 archivos nuevos bajo `docs/referencia/`, agrupados
por tema (no un archivo por etiqueta -- decisión explícita del
usuario, tras reconsiderar un primer borrador mecánico: ni 46
archivos diminutos ni pocos gigantes).

- **Extracción programática**, no manual: un script identifica cada
  sección por su encabezado `[ARCH:tag]`, agrupa según el mapa
  confirmado, y reconstruye `architecture.md` con SOLO las secciones
  sin etiqueta (el histórico cronológico puro). Verificado con
  rigor antes de borrar nada: los 49 títulos de sección originales
  se confirmaron presentes, uno a uno, en los archivos nuevos.
- `docs/architecture.md` (ahora solo histórico) se movió a
  `docs/historico/architecture.md`.
- `docs/relaciones_espaciales.md` y `docs/niveles_plantas.md` (fuentes
  normativas) se movieron a `docs/fuentes/`.
- Nuevo `docs/README.md`: índice de toda la documentación, explica
  dónde buscar cada cosa y cómo localizar una etiqueta `[ARCH:tag]`
  (`grep -rn` sobre `docs/`, ya no necesita saber en qué archivo
  concreto vive).
- Todas las referencias reales a las tres rutas movidas se
  actualizaron en todo el proyecto (código fuente, tests, INICIO.html,
  README.md, instalar.sh, resto de docs) -- verificado
  sistemáticamente que no queda ninguna referencia rota, y que las
  únicas menciones a las rutas antiguas que sobreviven están dentro
  del propio histórico (correctas ahí, describen la estructura de ese
  momento de la sesión).
- **Hallazgo real durante la propia reorganización**: el enlace
  "Referencia técnica" de `INICIO.html` apuntaba a
  `docs/historico/architecture.md` con la descripción "decisiones
  documentadas, [ARCH:tag]" -- ya no es así (esa referencia por
  etiqueta vive ahora en `docs/referencia/`, no en el histórico).
  Corregido para apuntar al nuevo `docs/README.md`.
- `CONTINUIDAD.md` (el mapa de archivos del proyecto) actualizado con
  la estructura completa nueva.
- Nuevo test de sanidad (`test_docs_readme_links_point_to_real_files`)
  que protege los enlaces de `docs/README.md`, mismo patrón que ya
  existía para `INICIO.html`.
- Verificado con la suite completa, pyflakes, mypy, y regenerado el
