...
def _room(room_id, level):
    return Room(id=room_id, name=room_id, room_type=RoomType.BEDROOM, dimensions=Dimensions(area_m2=10), level=level)
...
def test_floor_lot_uses_the_retranqueo_reduced_polygon_not_the_raw_one():
    ...
    assert area_reducida_esperada < poligono_real.area  # confirma que la reduccion es real, no un no-op
    ...
    assert floor_lot.area_edificable_real.polygon.area == pytest.approx(area_reducida_esperada)
    assert floor_lot.area_edificable_real.polygon.area < poligono_real.area  # nunca el crudo sin reducir
...
def test_floor_lot_real_boundary_shrinks_progressively_across_floors():
    ...
    lot = Lot(
        ...
        retranqueo_m=1.0, retranqueo_incremento_por_planta_m=1.0,
    )
    ...
    building = _use_case().execute(program, lot)
    area_pb = building.floors[NivelPlanta.PLANTA_BAJA].lot.area_edificable_real.polygon.area
    area_ps = building.floors[NivelPlanta.PLANTA_SUPERIOR].lot.area_edificable_real.polygon.area

    assert area_ps < area_pb  # planta superior: el mismo encogimiento progresivo que el rectangulo de trabajo
